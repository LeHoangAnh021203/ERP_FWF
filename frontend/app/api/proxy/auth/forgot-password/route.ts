...
const FORGOT_PASSWORD_ENDPOINT = getApiEndpoint('auth/forgot-password');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🧪 Auth Forgot Password Diagnostics:', {
    ...
    // Encode email for URL path
    const encodedEmail = encodeURIComponent(email);
    const apiUrl = `${FORGOT_PASSWORD_ENDPOINT}/${encodedEmail}`;

    console.log('🌐 Calling real API:', apiUrl)

    try {
      // Call the real API with email in URL path
      const apiResponse = await fetch(apiUrl, {
        ...
        // No body needed, email is in URL path
