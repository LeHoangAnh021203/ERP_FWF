...
export function ShiftStats({ shifts, employees, detailed = false }: ShiftStatsProps) {
  ...
  const approvalRate = totalShifts > 0 ? (approvedShifts / totalShifts) * 100 : 0
  ...
  // Position statistics
  const positionStats = shifts.reduce(
    (acc, shift) => {
  ...
  // Employee workload
  const employeeWorkload = shifts.reduce(
    (acc, shift) => {
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(employeeWorkload)
                .sort(([, a], [, b]) => (b as number) - (a as number))
                .map(([employee, count]) => (
                  <div key={employee} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="font-medium">{employee}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-600">{count as number} ca</span>
                      <div className="w-16 bg-gray-200 rounded-full h-2">
                        <div
                          className="bg-green-600 h-2 rounded-full"
                          style={{
                            width: `${Math.min(((count as number) / Math.max(...Object.values(employeeWorkload))) * 100, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
